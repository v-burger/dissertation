...
%The mean values with 95\% confidence intervals of 8 simulation runs are plotted for the service time distributions Deterministic and Hyper-exponential. In case of the Hyper-exponential distribution the service times are highly variant.
%
%\begin{figure}[tb]
%	\centering
% 	\includegraphics[width=0.5\textwidth]{figures/bw_n20_rho2_sim}
%  	\caption{Blocking probability dependent on the load of observed and cooperating system. Simulation with different service time distributions.}
%  	\label{fig:bw_n20_rho2_sim}
%\end{figure}
%
%In Figure~\ref{fig:bw_n20_rho2_sim}, which shows the available bandwidth of the observed system dependent on the load, simulation results are plotted for Deterministic distributed and highly variant Hyper-exponential distributed service times. For deterministic service times the analytic model fits the simulation results. If the service times are highly variant the observed system receives only slightly more bandwidth than in the model if it is overloaded. Hence, considering the available bandwidth the analytic model can be used to assess the system performance with general service time distributions.
