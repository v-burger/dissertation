...
Further, we summarize related work in the field of distributed active measurements of CDNs.
...
These business relationships are usually not open for public but they can be abstracted into two common types \cite{gao2001}.
The relationship between two ASs can be transit or peering.
\reffig{fig:aslevel:p2pcdn} shows AS topologies with ASs interconnected with inter-AS links.
Inter-AS links are either peering links or transit links.
A transit link is present if the customer AS pays the provider AS for transit service, i.e., the provider forwards the traffic of the customer and its customers. In peering relation the ASs have an agreement that they exchange each others traffic and the traffic of their customers, without paying each other. %Sibling-to-sibling are links between ASs of the same organization. These relations are defined in business agreements and kept secret, but they can be inferred by analyzing the routing between autonomous systems.

The two most popular content delivery concepts are P2P networks and CDNs.
To deliver content, it has to be transported from storage resources to clients that consume the content.
\reffig{fig:aslevel:p2pcdn} shows the autonomous systems topology for a P2P network, and a CDN.
In case of P2P networks, c.f., \reffig{fig:aslevel:p2p}, peers contribute storage resources by serving chunks of data.
To enable exchange of data, a tracker keeps track of the peers sharing chunks of a file.
In the past P2P networks where responsible for the most part of traffic.
Due to its
The largest amount of traffic is now transported by CDNs.
In CDNs, c.f., \reffig{fig:aslevel:cdn}, the storage resources are managed data centers or caches.
...
While the amount of traffic transported over peer-to-peer networks remained about the same, the traffic transported by CDNs has increased exponentially \cite{cisco2016}.
